...
namespace REslava.Result.SourceGenerators.Generators.SmartEndpoints.CodeGeneration
{
    ...
    public class SmartEndpointExtensionGenerator : ICodeGenerator
    {
        public SourceText GenerateCode(Compilation compilation, object config)
        {
            ...
            if (!endpoints.Any())
            {
                System.Diagnostics.Debug.WriteLine("⚠️ No endpoints found");
                return SourceText.From("// No SmartEndpoints found", Encoding.UTF8);
            }
            ...
            System.Diagnostics.Debug.WriteLine("🚀 SmartEndpointExtensionGenerator.GenerateCode completed!");
            return result;
        }
            ...
            ControllerMetadata controller)
        {
            ...
            // Enhanced detection for OneOf4 specifically
            var isOneOf4 = isOneOf && 
                           returnType is INamedTypeSymbol namedTypeForOneOf4 && 
                           namedTypeForOneOf4.TypeArguments.Length == 4;
            ...
            // Add OneOf4 specific metadata
            if (isOneOf4)
            {
                endpoint.IsOneOf4 = true;
                System.Diagnostics.Debug.WriteLine($"🔥 SmartEndpoints: Detected OneOf4 method: {methodSymbol.Name}");
            }
        ...
        {
            // UserController → /user, ProductService → /product
            var name = className
                .Replace("Controller", "")
                .Replace("Service", "")
                .Replace("Endpoints", "");
        ...
        {
            // GetUser(int id) → /user/{id}
            // GetUserById(int id) → /user/{id}
            // CreateUser → /user
            // UpdateUser(int id) → /user/{id}
        ...
        private void GenerateEndpointMappingSimple(StringBuilder builder, EndpointMetadata endpoint)
        {
            ...
            else if (endpoint.ReturnType.Contains("OneOf<") && endpoint.ReturnType.Count(c => c == ',') == 3)
            {
                // OneOf<T1,T2,T3,T4> - use OneOf4Extensions
                builder.AppendLine($"                    return result.ToIResult();");
            }
