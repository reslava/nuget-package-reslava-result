...
{
    /// <summary>
    /// Generates OneOf3ToIResult extension methods - FIXED
    /// </summary>
    public class OneOf3ToIResultExtensionGenerator : ICodeGenerator
    {
        public SourceText GenerateCode(Compilation compilation, object config)
        {
            ...
            builder.AppendLine("    /// Extension methods for converting OneOf with 3 types to IResult.");
            builder.AppendLine("    /// </summary>");
            builder.AppendLine("    public static class OneOf3Extensions");
            builder.AppendLine("    {");
            
            // FIX: Add generic type parameters <T1, T2, T3>
