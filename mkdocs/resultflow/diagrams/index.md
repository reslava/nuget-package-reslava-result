...
flowchart LR
  ...
  N1_Validate["Validate"]:::bind
  N2_ReserveInventory["ReserveInventory"]:::bind
  N3_ProcessPayment["ProcessPayment"]:::bind
...
flowchart LR
  ...
  subgraph sg_N0_ValidateUser["ValidateUser"]
    ...
    N0_0_Bind["Bind<br/>User"]:::bind
    N0_0_Bind -->|ok| N0_1_Bind
    N0_0_Bind -->|"UserInactiveError"| FAIL
    N0_1_Bind["Bind<br/>User"]:::bind
...
## `Legend` — Node type reference (v1.47.2)

One `Legend` constant is emitted per `*_Flows` class (not per method). Paste it into any Mermaid renderer to review all node types and colors at a glance.

```mermaid
flowchart LR
    S[ ]:::entry ==> E["Entry"]:::operation
    E ~~~ B["Bind"]:::bind
    B ~~~ M["Map"]:::map
    M ~~~ G["Guard"]:::gatekeeper
    G ~~~ T["Tap"]:::sideeffect
    T ~~~ X{{"Match"}}:::terminal
    X ~~~ SUC([success]):::success
    X ~~~ F([fail]):::failure
    F ~~~ A["⚡ = async"]:::note
    classDef entry      fill:none,stroke:none
    classDef operation  fill:#fef0e3,color:#b86a1c
