...
public class ResultFlowDiagramDeliveryTests
{
    ...
    [TestMethod]
    public void ConfigLoader_ParsesLinkMode_Vscode()
    {
        var json = @"{
  ""linkMode"": ""vscode"",
  ""mappings"": []
}";
        ResultFlowConfigLoader.TryLoad(json, out _, out var linkMode);
        Assert.AreEqual("vscode", linkMode);
    }

    [TestMethod]
    public void ConfigLoader_ParsesLinkMode_None()
    {
        var json = @"{
  ""linkMode"": ""none"",
  ""mappings"": []
        ...
        ResultFlowConfigLoader.TryLoad(json, out _, out var linkMode);
        Assert.AreEqual("none", linkMode);
    }

    [TestMethod]
    public void ConfigLoader_MissingLinkMode_ReturnsNull()
    {
        var json = @"{ ""mappings"": [] }";
        ResultFlowConfigLoader.TryLoad(json, out _, out var linkMode);
        Assert.IsNull(linkMode, "linkMode must be null when not present in config");
    }
}
