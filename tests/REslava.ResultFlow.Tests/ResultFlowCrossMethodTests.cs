...
public class ResultFlowCrossMethodTests
{
    ...
    public void CrossMethod_InnerGatekeeper_HasFailEdge()
    {
        ...
        Assert.IsTrue(output.Contains("Failure"),     "Inner Ensure has its own Failure node");
        Assert.IsTrue(output.Contains("fail"),        "Inner Ensure has fail edge");
    }
    ...
    public void CrossMethod_MutualRecursion_TerminatesCleanly()
    {
        ...
        // Cycle guard (HashSet<string> by name) must stop re-entry.
    ...
    // ── 6. Ambiguity guard: 2 methods with same name → no subgraph ───────────
    [TestMethod]
    public void CrossMethod_AmbiguousMethodName_NoSubgraph()
    {
        // Two overloads named Validate → name lookup finds 2 matches → skip tracing
        var source = CreateSource("OrderService", "Process",
            "CreateOrder().Bind(x => Validate(x))",
            extraMethods: @"
        static Result<Order> CreateOrder() => Result<Order>.Ok(new Order());
        static Result<Order> Validate(Order o) => Result<Order>.Ok(o);
        static Result<Order> Validate(Order o, string hint) => Result<Order>.Ok(o);");

        var output = RunGenerator(source);

        Assert.IsFalse(output.Contains("subgraph sg_"),
            "Ambiguous method name (2+ matches) should not expand into subgraph");
        Assert.IsTrue(output.Contains("Validate"),
            "Validate still shown as leaf node label");
    }

    // ── 7. Non-lambda arg (method group) → no subgraph expansion ─────────────
    [TestMethod]
    ...
    // ── 8. MaxDepth = 1: first level expands, nested level does not ──────────
    [TestMethod]
    public void CrossMethod_MaxDepthOne_OnlyFirstLevelExpands()
    {
        ...
        static Result<Order> DeepCheck(Order o) =>
            Result<Order>.Ok(o).Ensure(x => x.Id > 0, ""x"");");
