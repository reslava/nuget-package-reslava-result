...
namespace REslava.Result.SourceGenerators.Tests.ResultToIResult;
...
";
        ...
        // Should NOT generate attributes when no Result types are found (conditional generation)
        var attributeFiles = generatorResult.GeneratedSources
            .Where(f => f.HintName.Contains("Attribute"));
        
        Assert.IsTrue(attributeFiles.Count() == 0, "Should NOT generate attributes when no Result types are found");
    }
}
