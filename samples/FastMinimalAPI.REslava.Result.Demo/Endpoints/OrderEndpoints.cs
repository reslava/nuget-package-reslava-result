...
/// Uses library domain errors (NotFoundError, ConflictError, ValidationError).
...
public static class OrderEndpoints
{
    public static void MapOrderEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        // POST /api/orders - Create new order (demonstrates OneOf4 - complex pattern!)
        group.MapPost("/", async ([FromBody] CreateOrderRequest request, OrderService orderService) =>
        {
            var result = await orderService.CreateOrderAsync(request);

            // OneOf4 pattern: NotFoundError | ConflictError | ValidationError | OrderResponse
            return result.Match(
                case1: notFound => Results.NotFound(new {
                    success = false,
                    error = notFound.Message,
                ...
                case2: conflict => Results.Conflict(new {
                    success = false,
                    error = conflict.Message,
                    errorType = "Conflict"
                }),
                case3: validation => Results.UnprocessableEntity(new {
                    success = false,
        ...
        .WithDescription("Complex validation: checks user exists, user active, stock available, order not empty (3 error types + success)")
        .Produces<object>(201)
        .Produces<object>(404)
        .Produces<object>(409)
        .Produces<object>(422);
            ...
            OrderService orderService) =>
        {
            ...
            return result.Match(
                ...
                case2: validation => Results.UnprocessableEntity(new {
                    success = false,
                    error = validation.Message,
                    field = validation.FieldName
                }),
