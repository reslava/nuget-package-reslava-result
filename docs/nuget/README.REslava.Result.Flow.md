...
![Cross-method pipeline tracing — [ResultFlow(MaxDepth = 2)] expands Bind lambdas into named subgraphs](images/OrderService_PlaceOrderCross.svg)

**Architecture layer view** — Domain / Application / Infrastructure boundaries:

![Architecture layer view — Application → Domain layers, color-coded subgraphs](images/OrderService_PlaceOrderCross_LayerView.svg)
...
![Match multi-branch fan-out — hexagon node with one typed error edge per branch](images/MatchDemo_ConfirmOrder.svg)
...
![Dark theme — cross-method pipeline with dark classDef palette](images/FulfillmentService_FulfillOrder.svg)
