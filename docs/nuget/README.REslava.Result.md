...
- **Pipeline visualization** — `[ResultFlow]` auto-generates Mermaid diagrams from your code (separate package); install [REslava.Result Extensions](https://marketplace.visualstudio.com/items?itemName=reslava.reslava-result-extensions) from the VS Code Marketplace for `▶ Open diagram preview` CodeLens
